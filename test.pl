...
# Test undefined keys.
{
	local $SIG{__WARN__} = sub { $ok = 1 if $_[0] =~ /^Use of uninitialized value/ };
	local $^W = 1;

	$ok = 0; 
	$x = undef;
	not defined $h{$x} and $ok or print 'not ';
	print "ok 16\n";

	$ok = 0;
	not exists $h{$x} and $ok or print 'not ';
	print "ok 17\n";
}
...
@k = keys %h; @v = values %h;
$k[0] eq 'dog' and $k[1] eq 'cat' and $k[2] eq 'cat' and $k[3] eq 'dog' and
	$v[0] eq 'perro' and $v[1] eq 'gato' and $v[2] eq 'chat' and $v[3] eq 'chien' or
	print 'not ';
print "ok 25\n";
